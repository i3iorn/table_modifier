...
class Detector(ABC):
    ...
    def detect(self, values: List[str]) -> float:
        ...
        self.logger.debug(f"Detector {self.type_name()} final score: {score:.2f}")
        self.logger.debug(f"Checks done: {check_done}")
        ...
        # Normalize the score based on the depth of the detector, this is done to
        # ensure that deeper detectors are given an advantage as they are more specific.
